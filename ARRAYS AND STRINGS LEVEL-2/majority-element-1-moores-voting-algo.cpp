...
int majorityElement1MooreVotingAlgo(vector<int> arr) {
	int potentialCandidateForMajorityElement = validCandidate(arr); //this will return the potential candidate for majority element

	//now count the freq of potentialCandidateForMajorityElement
	int count = 0;
	for (int ele : arr) {
		if (ele == potentialCandidateForMajorityElement) count++;
	}

	// check if the count is greater than arr.size()/2
	if (count > arr.size() / 2) {
		return potentialCandidateForMajorityElement;
	}
...
// 3. Note : solve the problem in linear time and in O(1) space.
	//*********************************
	vector<int> arr = {1, 1, 1, 1, 1, 5, 5, 3, 6};
