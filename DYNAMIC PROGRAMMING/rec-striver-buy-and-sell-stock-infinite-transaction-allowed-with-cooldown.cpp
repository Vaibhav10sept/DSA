...
int rec(int ind, bool buy, vector<int> arr, vector<vector<long>> &memo) {
	...
	if (buy) { //you can buy not sell
		...
		int profitSell = arr[ind] + rec(ind + 2, true, arr, memo); //ind+2 this is the only change(think, WV)
...
{
	/***************************
	 * NOTE: same code with slight modification as in rec-striver-buy-and-sell-stock-infinite-transaction-allowed
