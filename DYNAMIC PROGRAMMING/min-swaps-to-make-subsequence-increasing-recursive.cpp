...
int solve(vector<int> arr1, vector<int> arr2, int idx, bool swapped) {
	...
	if (arr1[idx] > prev1 and arr2[idx] > prev2) {
		//no swap
		ans = solve(arr1, arr2, idx + 1, false);
	}
	if (arr1[idx] > prev2 and arr2[idx] > prev1) { //see the if clearly, think, WV
		//swap happened
		ans = min(ans, 1 + solve(arr1, arr2, idx + 1, true)); // + 1 bcoz , swapping hue to count of swapping is incresed by 1
	}

	return ans; //ans is the min no of swapping.
}

int minSwapToMakeSubsequenceIncreasing(vector<int> arr1, vector<int> arr2) {
	arr1.insert(arr1.begin(), -1);
	arr2.insert(arr2.begin(), -1);
	return solve(arr1, arr2, 1, false);
}
