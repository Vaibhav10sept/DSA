...
void transformToLeftClonedTree(Node* node) {
	if (node == NULL) return;
...
Node* transformBackToOriginalTree(Node* node) {
	...
	node->left = leftNormalRoot;
	node->right = rightNormalRoot;
...
{
	//******************************************
	// NOTE: THIS CODE IS NOT WORKING I DON'T KNOW WHY
	//****************************************
	vector<int> arr = {50, 25, 12, -1, -1, 37, 30, -1, -1, -1, 75, 62, -1, 70, -1, -1, 87, -1, -1};
	Node* root = construct(arr);
	display(root);
	transformToLeftClonedTree(root);
	cout << "after clonning to left clonned tree" << endl;
	display(root);
	cout << "after transforming back to original tree" << endl;
	root = transformBackToOriginalTree(root);
	display(root);
