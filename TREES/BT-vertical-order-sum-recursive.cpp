...
void dfs(Node* node, int horizontalLevel, vector<int> &ans) {
	if (node == NULL) return;

	ans[horizontalLevel] += node->data;

	dfs(node->left, horizontalLevel - 1, ans);
	dfs(node->right, horizontalLevel + 1, ans);
}
...
vector<int> verticalOrderSum(Node* root) {
	...
	dfs(root, abs(minHorizontalLevel), ans);
...
int main()
{
	...
	USED QUESTION: width of a binary tree
	VIDEO LINK: https://www.youtube.com/watch?v=3N4PXGyfYiI&list=PL-Jc9J83PIiHgjQ9wfJ8w-rXU368xNX4L&index=33
	*/
	vector<int> arr = {50, 25, 12, -1, -1, 37, 30, -1, -1, -1, 75, 62, -1, 70, -1, -1, 87, -1, -1};
	Node* root = construct(arr);
	display(root);
	cout << "vertical order sum: " << endl;
	vector<int> res = verticalOrderSum(root);
	for (auto ele : res) {
		cout << ele << " ";
	}
}