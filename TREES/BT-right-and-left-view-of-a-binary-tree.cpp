...
void rightSideViewRecursionHelper(Node* node, int level, vector<int> &res) {
	//NOTE: ye level line wise traversal vale level hai
	if (node == NULL) return;

	//this is the tricky part all logic lie here(watch video)
	//NOTE: if i travel right to left then first node in every level will be my node for right side view
	if (res.size() == level) res.push_back(node->data);

	rightSideViewRecursionHelper(node->right, level + 1, res);
	rightSideViewRecursionHelper(node->left, level + 1, res);
}

void leftSideViewRecursionHelper(Node* node, int level, vector<int> &res) {
	//NOTE: ye level line wise traversal vale level hai
	if (node == NULL) return;
	...
	leftSideViewRecursionHelper(node->right, level + 1, res);
}

vector<int> rightSideView(Node* root) {
	vector<int> res;
	rightSideViewRecursionHelper(root, 0, res);
	return res;
}

vector<int> leftSideView(Node* root) {
	vector<int> res;
	leftSideViewRecursionHelper(root, 0, res);
	return res;
}
...
int main()
{
	/*
	PREREQUISITE:
	VIDEO LINK: https://www.youtube.com/watch?v=KV4mRzTjlAk
	*/
