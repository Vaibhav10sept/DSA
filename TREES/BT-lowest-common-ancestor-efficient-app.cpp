...
bool lowestCommonAncestorHelper(Node* node, Node* &lowestCommonAncestor, int data1, int data2) {
	if (node == NULL) return false;

	bool leftAns = lowestCommonAncestorHelper(node->left, lowestCommonAncestor, data1, data2);
	bool rightAns = lowestCommonAncestorHelper(node->right, lowestCommonAncestor, data1, data2);
	bool self = (node->data == data1 or node->data == data2);

	if ((leftAns and rightAns) or (leftAns and self) or (rightAns and self)) {
		//means, yhi lca hai
		lowestCommonAncestor = node;
		return true;
	}

	if (node->data == data1 or node->data == data2) return true;

	//agr left or right or seft mese kisi me bhi true aega toh true return hoga(think, watch video)
	return leftAns or rightAns or self;
}

int lowestCommonAncestor(Node* root, int data1, int data2) {
	Node* lowestCommonAncestor = NULL;

	lowestCommonAncestorHelper(root, lowestCommonAncestor, data1, data2);
...
{
	/*
	VIDEO LINK: https://www.youtube.com/watch?v=9n4v9fOxI5w&list=PL-Jc9J83PIiHgjQ9wfJ8w-rXU368xNX4L&index=59
	*/
	vector<int> arr = {50, 25, 12, -1, -1, 37, 30, -1, -1, -1, 75, 62, -1, 70, -1, -1, 87, -1, -1};
	Node* root = construct(arr);
	display(root);
	cout << "res " << endl;
	cout << lowestCommonAncestor(root, 62, 70);
}