...
void countOfAllSingleChildNodesHelper(Node* node, int &ans) {
	if (node == NULL) return;
	...
	countOfAllSingleChildNodesHelper(node->left, ans);
	countOfAllSingleChildNodesHelper(node->right, ans);
}

int countOfAllSingleChildNodes(Node* root) {
	int ans = 0;
	countOfAllSingleChildNodesHelper(root, ans);
	return ans;
}
...
{
	/*
	VIDEO LINK: https://www.youtube.com/watch?v=pPPa8aHnzGU&list=PL-Jc9J83PIiHgjQ9wfJ8w-rXU368xNX4L&index=42
	*/
	vector<int> arr = {50, 25, 12, -1, -1, 37, 30, -1, -1, -1, 75, 62, -1, 70, -1, -1, 87, -1, -1};
	Node* root = construct(arr);
	display(root);
	cout << "count of all single nodes:" << endl;
	cout << countOfAllSingleChildNodes(root);
}