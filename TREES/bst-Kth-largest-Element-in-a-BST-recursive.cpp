...
int solve(Node* node, int k) {
	...
	//right call
	//we do reverse inorder traversal
	int right = solve(node->right, k);
	if (right != 0) return right; //means right se answer mil gaya
	...
	return left; //think left vale case me if nhi lgaya but above left me if check lgaya tha.
}

int kthLargestElementRecursive(Node* root, int k) {
	//reverse inorder traversal of BST gives elements in decreasing fashion.
	return solve(root, k);
}
...
int main()
{
	...
	*/
	...
	display(root);
	cout << "answer" << endl;
	cout << kthLargestElementRecursive(root, k);
}