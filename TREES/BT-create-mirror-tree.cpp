...
Node* createMirrorHelper(Node* root) {
	//I wrote this without video
	if (root == NULL) return NULL;
	//BC ends

	Node* tempLeft = root->left;
	root->left = createMirrorHelper(root->right);
	root->right = createMirrorHelper(tempLeft);

	return root;
}

void createMirror(Node* root) {
	root = createMirrorHelper(root);
}
