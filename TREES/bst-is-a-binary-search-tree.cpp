...
utilData isBinarySearchTree(Node* node) {
	if (node == NULL) {
		utilData bc;
		return bc;
	}

	utilData leftAns = isBinarySearchTree(node->left);
	utilData rightAns = isBinarySearchTree(node->right);

	utilData currAns;
	currAns.min = std::min(node->data, std::min(leftAns.min, rightAns.min));
	currAns.max = std::max(node->data, std::max(leftAns.max, rightAns.max));

	bool isCurrNodeBST = (node->data >= leftAns.max && node->data <= rightAns.min);
...
{
	//********************************
	//LOGIC: LEFT KE MAXIMUM <= NODE->DATA <= RIGHT KE MINIMUM
	...
	utilData ans = isBinarySearchTree(root);
