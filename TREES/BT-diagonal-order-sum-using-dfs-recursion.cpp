...
void dfs(Node* node, int diagonalNumber, vector<int> &ans) {
	if (node == NULL) return;

	if (diagonalNumber == ans.size()) ans.push_back(0);
	ans[diagonalNumber] += node->data;

	//left call: isme diagonal number +1 hoga
	dfs(node->left, diagonalNumber + 1, ans);
	//right call: isme diagonal number change nhi hoga (think)
	dfs(node->right, diagonalNumber, ans);
}

vector<int> diagonalOrderSum(Node* root) {
...
{
	/*
	PREREQUISITE: diagonal order traversal
	VIDEO LINK: https://www.youtube.com/watch?v=RDlYv8YuTXQ&list=PL-Jc9J83PIiHgjQ9wfJ8w-rXU368xNX4L&index=35
	*/
	vector<int> arr = {50, 25, 12, -1, -1, 37, 30, -1, -1, -1, 75, 62, -1, 70, -1, -1, 87, -1, -1};
	Node* root = construct(arr);
	display(root);
	cout << "diagonal Order sum " << endl;
	vector<int> res = diagonalOrderSum(root);
	for (int ele : res) {
		cout << ele << " ";
	}
}