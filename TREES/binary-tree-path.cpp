...
void rec(Node* node, string path, vector<string> &ans) {
	if (node == NULL) return;
	...
	// if (node->data == 2) cout << "tes " << path << endl;
	rec(node->left, path + to_string(node->data) + "->", ans);
	rec(node->right, path + to_string(node->data) + "->", ans);
}
