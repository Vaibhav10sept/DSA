...
void listOfAllSingleNodeHelper(Node* node, vector<int> &ans) {
	if (node == NULL) return;
	...
	listOfAllSingleNodeHelper(node->left, ans);
	listOfAllSingleNodeHelper(node->right, ans);
}

vector<int> listOfAllSingleNode(Node* root) {
	vector<int> ans;
	listOfAllSingleNodeHelper(root, ans);
	return ans;
}
...
int main()
{
	...
	*/
	...
	display(root);
	cout << "list of all single nodes:" << endl;
	vector<int> ans = listOfAllSingleNode(root);
