...
int lowestCommonAncestorSirApproach(Node* node, int val1, int val2) {
	// if (node == NULL) {
	// 	return -1;
	// }
	...
	if (val1 < node->data && val2 < node->data) {
		//move left
		return lowestCommonAncestorSirApproach(node->left, val1, val2);
	}
	else if (val1 > node->data && val2 > node->data) {
		//move right
		return lowestCommonAncestorSirApproach(node->right, val1, val2);
	}
	else {
		//means ek chota hai or doosra bada hai
...
int main()
{
	...
	display(root);
	cout << "lowest common ancestor: " << lowestCommonAncestorSirApproach(root, 62, 87);
}