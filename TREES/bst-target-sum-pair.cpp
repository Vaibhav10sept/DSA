...
void targetSumPair(Node* node, int target, Node* root) {
	if (node == NULL) return;

	targetSumPair(node->left, target, root);

	//inorder phase start
	int comp = target - node->data;
	if (comp > node->data)// this will remove duplicacy(think)(or try removing this "if" condition)
	{
		if (find(root, comp)) {
	...
	targetSumPair(node->right, target, root);
