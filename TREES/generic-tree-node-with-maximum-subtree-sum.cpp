...
int nodeWithMaxSubtreeSum(Node* root, int *ans, Node** node) {
	int sum = 0;
	for (Node* child : root->children) {
		sum += nodeWithMaxSubtreeSum(child, ans, node);
	}
	sum += root->data;
	if (sum > *ans) {
		*ans = sum;
...
int main()
{
	vector<int> arr = {10, 20, -50, -1, 60, -1, -1, 30, -70, -1, 80, -1, 90,
	                   -1, -1, 40, -100, -1, -1, -1
	                  };
	...
	int ans = INT_MIN;
	Node* node = NULL;
	nodeWithMaxSubtreeSum(root, &ans, &node);
	cout  << "node " << node->data << " " << "sum " << ans;
}