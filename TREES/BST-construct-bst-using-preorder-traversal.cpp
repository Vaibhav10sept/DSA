...
Node* HelperFunction(vector<int> preorder, int leftRange, int rightRange) {
	static int idx = 0;
	//BASE CASE:
	if (idx >= preorder.size() or preorder[idx] < leftRange or preorder[idx] > rightRange) return NULL;

	Node* node = new Node(preorder[idx]);
	idx++;
	//recursive calls
	node->left = HelperFunction(preorder, leftRange, node->data);
	node->right = HelperFunction(preorder, node->data, rightRange);
...
Node* constructBSTFrompreorderTraversal(vector<int> preorder) {
	...
	return HelperFunction(preorder, leftRange, rightRange);
}

int main()
{
	/*
	PREREQUISITE: construct BST from inorder traversal
	VIDEO LINK: https://www.youtube.com/watch?v=UAsLKuEMhsQ&list=PL-Jc9J83PIiHgjQ9wfJ8w-rXU368xNX4L&index=16
	*/
	vector<int> preorder = {8, 5, 1, 7, 10, 12};
	Node* root = constructBSTFrompreorderTraversal(preorder);
	display(root);
}