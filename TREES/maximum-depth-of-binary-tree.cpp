...
int maxDepth(Node* node) {
	if (node == NULL) {
		return 0; //rem in terms of node nikal rhe isliye 0 return kia, agr in terms of edge nikal rhe hote to -1 return krte(think, cs, dry run)
	}

	int leftHeight = maxDepth(node->left);
	int rightHeight = maxDepath(node->right);
