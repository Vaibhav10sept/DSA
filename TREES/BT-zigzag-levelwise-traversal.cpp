...
vector<vector<int>> zigzagTraversal(Node* root) {
	...
	while (!q.empty()) {
		...
		for (int i = 0; i < size; i++) {
			...
			int index = leftToRight ? i : (size - i - 1); //think, easy CS
			row[index] = temp->data;

			//add left and right node of temp
			if (temp->left) q.push(temp->left);
			if (temp->right) q.push(temp->right);
		}
		//after completing one level, switch leftToRight and push row in ans
...
{
	/*
	QUESTION: https://takeuforward.org/data-structure/zig-zag-traversal-of-binary-tree/
	VIDEO LINK:
