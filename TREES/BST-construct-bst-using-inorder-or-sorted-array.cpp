...
Node* constructBSTFromInorderTraversal(vector<int> arr, int low, int high) {
	if (low > high) return NULL;

	int mid = (low + high) / 2;

	Node* leftChild = constructBSTFromInorderTraversal(arr, low, mid - 1);
	Node* rightChild = constructBSTFromInorderTraversal(arr, mid + 1, high);
...
int main()
{
	...
	*/
	...
	Node* root = constructBSTFromInorderTraversal(arr, 0, arr.size() - 1);
	display(root);
}