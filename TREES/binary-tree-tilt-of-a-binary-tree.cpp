...
int tiltOfABinaryTree(Node* node, int &ans) {
	if (node == NULL) return 0;

	int leftSum = tiltOfABinaryTree(node->left, ans);
	int rightSum = tiltOfABinaryTree(node->right, ans);
...
int main()
{
	...
	display(root);
	int ans = 0;
	tiltOfABinaryTree(root, ans);
	cout << "tilt of binary tree is: " << ans;
}