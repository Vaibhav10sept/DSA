...
public:
	...
	void removeAfter(Node* node, int val) {
		Node* temp  = head;
		while (temp) {
			if (temp == node) { //node found
				if (temp->next == NULL) { //temp is tail
					//do nothing, coz tail ke baad kuch nhi hota
				}
				else {
...
{
	/*
	VIDEO LINK: https://www.youtube.com/watch?v=ktSIOF2q3m8&list=PL-Jc9J83PIiGRqcfZxxgOKovgLVd3znnq&index=43
	*/
	DoublyLinkedList ll;
	ll.addFirst(6);
	ll.addFirst(5);
