...
bool isValid(vector < vector < char >> & board, int row, int col, char c) {
	for (int i = 0; i < 9; i++) {
		...
		//this if ka logic mujhe smjh aa gya, think, CS
				...
				for (char c = '1'; c <= '9'; c++) {
					if (isValid(board, i, j, c)) {
						...
						if (solveSudoku(board))
							return true; //kyoki hme ek hi sudoku fill krke deni h
						else
							board[i][j] = '.';
					}
				}
				//1 to 9, sare numbers try lie but koi fit nhi hua to return false.
				return false;
			}
		}
	}
