...
bool isSameTree(Node* first, Node* second) {
	//base case
	if (first == NULL or second == NULL) return first == second;

	bool leftAns = isSameTree(first->left, second->left);
	bool rightAns = isSameTree(first->right, second->right);

	return (leftAns and rightAns and first->data == second->data);
}
...
bool isSubtree(Node* root, Node* subRoot) {
	...
	if (root->data == subRoot->data) return isSameTree(root, subRoot);
...
int main()
{
	...
	VIDEO LINK:
	*/
	vector<int> rootarr = {3, 4, 5, 1, 2, -1, -1, -1, -1, 0};
	vector<int> subRootarr = {4, 1, 2};
	Node* root = constructorForLeetcode(rootarr);
	Node* subRoot = constructorForLeetcode(subRootarr);
	display(root);
	cout << endl;
	display(subRoot);
