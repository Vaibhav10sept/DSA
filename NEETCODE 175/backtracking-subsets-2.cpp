...
void subsetsWithDuplicatesBacktracking(vector<int> arr, int idx, vector<int> ans, vector<vector<int>> &res, int sum) {
	if (idx >= arr.size()) {
		// res.push_back(ans);
	...
	subsetsWithDuplicatesBacktracking(arr, idx + 1, ans, res, sum + arr[idx]);
	...
	subsetsWithDuplicatesBacktracking(arr, idx + 1, ans, res, sum);
}
...
vector<vector<int>> subsetsWithDuplicates(vector<int>& arr) {
	...
	subsetsWithDuplicatesBacktracking(arr, 0, ans, res, 0);
	return res;
}
...
int main() {
	...
	*********************************/
	...
	vector<vector<int>> ans = subsetsWithDuplicates(arr);
	for (auto vec : ans) {
		for (auto ele : vec) {
			cout << ele << " ";
		}
