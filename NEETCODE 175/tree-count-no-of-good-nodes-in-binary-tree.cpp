...
void countOfGoodNodesHelper(Node* node, int &count, int maxValueSoFar) {
	if (node == NULL) return;
	//NOTE: we are doing things in preorder
	if (node->data >= maxValueSoFar) {
		count++;
	}

	//recursive call: note: we are traversing in preorder
	countOfGoodNodesHelper(node->left, count, max(maxValueSoFar, node->data));
	countOfGoodNodesHelper(node->right, count, max(maxValueSoFar, node->data));
}

int countOfGoodNodes(Node* root) {
	int count = 0;
	countOfGoodNodesHelper(root, count, INT_MIN);
	return count;
}
...
int main()
{
	...
	VIDEO LINK: https://www.youtube.com/watch?v=7cp5imvDzl4
	*/
	vector<int> arr = {3, 1, 4, 3, -1, 1, 5};

	Node* root = constructorForLeetcode(arr);
	display(root);
	cout << countOfGoodNodes(root);
}