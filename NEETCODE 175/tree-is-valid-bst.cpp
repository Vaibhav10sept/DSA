...
bool isValidBSTHelper(Node *node, long long int leftRange, long long int rightRange) {
	if (node == NULL)
		return true;
	...
	return isValidBSTHelper(node->left, leftRange, node->data) and isValidBSTHelper(node->right, node->data, rightRange);
}

bool isValidBST(Node* root) {
	// INT_MIN OR INT_MAX nhi use kr skte, leetcode pe test case fail hoge
	long long int rightRange = pow(2, 31);
	long long int leftRange = -1 * pow(2, 31) - 1;
	return isValidBSTHelper(root, leftRange, rightRange);
}
